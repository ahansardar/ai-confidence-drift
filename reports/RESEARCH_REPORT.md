...
The original per-step drift detector did not flag any of those six errors at the uncorrupted step, where no history exists. I then trained a sequence-level detector to predict whether the *original* classification was wrong after six controlled probes. With probes seeded from document text, it flagged five of the six errors and 44 correct high-confidence predictions. Nested validation on the training corpus caught four of six errors. A separate immediate review rule covered all six, with 78 correct cases sent to review. These counts are too small, and the review load too high, to claim the detector is ready for deployment.
...
The per-step C detector has no history at step 0, so it cannot answer whether the original prediction was wrong using drift. I built a second detector whose target is precisely that original error. For each of the 1,315 base-training documents, I generated six confidence readings from a base classifier trained on other documents in a stratified fold. The probe random seed comes from the document text, so changing a caller-supplied ID does not change its score. I summarized the complete trajectory, its class switches, original confidence and predicted class, and document length into one row per document. The error detector is a balanced Logistic Regression model. Its review threshold is 75% of the lowest score assigned to a high-confidence training error in five meta-model training folds. If a training fold has no such error, it issues no alerts rather than treating every case as an alert. This margin was chosen during development; it favors recall at the cost of false alarms. No test-set label is used to fit the model or compute its threshold.

At inference, `score_documents` in `src/sequence_error_detector.py` produces the six probes from unlabeled text, computes the same features, and returns the original classification's error score and flag. The score is not a calibrated probability. The flag is available only *after* those probes, not at the initial prediction. The command line scorer validates IDs and text; the frozen-model evaluator reports results and text overlap for a newly labeled corpus. I assessed the procedure in five outer folds over the base-training documents, rebuilding the base classifier, features, detector, and threshold without each outer validation document. Finally I evaluated the saved detector on the separate 439-document base-model test set. This detector was developed after inspecting that test set's earlier failure, so the latter result is exploratory rather than prospective confirmation.
...
- Six of 100 high-confidence predictions were wrong, accounting for 7.2% of the uncorrupted base model's errors. The per-step detector missed all six at step 0. The text-stable sequence-level detector flagged five of six after six probes and flagged 44 correct predictions. The separate immediate review policy routed all six and 78 correct predictions to review.
...
The **sequence-level original-error detector** uses the completed six-probe trajectory instead of trying to predict the original error at step 0. On the 439-document test set, it flagged 5 of 6 high-confidence errors and 44 of the 94 correct high-confidence predictions, for 49 alerts and 10.2% alert precision. Its high-confidence ROC-AUC was 0.819. A start-only model with original confidence, predicted class, and document length also flagged 5/6, with 43 false alarms and ROC-AUC 0.780. The trajectory improved ranking in this sample but did not improve recall at the chosen threshold. In a five-fold nested validation on the base-training corpus, the complete procedure flagged 4 of 6 observed high-confidence errors and 108 correct predictions. One outer fold had no high-confidence training errors, so its threshold withheld alerts and missed two validation errors. The old zero-threshold fallback would have flagged every high-confidence case in that fold and misleadingly counted those errors as detected. Those results are in `results/metrics/sequence_error_detector_analysis.json`. The primary saved model is `models/drift_detector_best.joblib`; `models/drift_detector_per_step.joblib` preserves the earlier A/B/C model. The detector is usable for demonstrations and further evaluation, but its small error sample, high review load, unstable threshold, and retrospective test prevent a deployment-readiness claim.
...
- Collect a new labeled corpus before inspecting its scores, then set a recall and review-capacity target. Reduce the sequence-level detector's false alarms and test whether its six-probe method works beyond this corpus. For immediate step-0 coverage, the review policy remains costly.
